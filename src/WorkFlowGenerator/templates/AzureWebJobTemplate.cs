...
public static class AzureWebJobTemplate
{
        ...
        string publishProfileName)
    {
        ...
        root.env = new()
        {
            { "WEBJOB_NAME", azure_resource_name },
            { "AZURE_WEBAPP_NAME", azure_resource_name },
            { "AZURE_WEBJOB_PACKAGE_PATH", package_path },
        ...
        Step[] buildSteps = new Step[] {
            ...
            DotNetStepHelper.AddDotNetPublishStep("Publish","${{ env.WORKING_DIRECTORY }}", "${{ env.CONFIGURATION }}", "${{ env.AZURE_WEBJOB_PACKAGE_PATH }}", "-r win-x86 --self-contained true"),
            AzureStepHelper.AddAzureWebAppDeployStep("Deploy to Azure Web App","${{ env.AZURE_WEBAPP_NAME }}", "./publish", publishProfileName)
        };
        root.jobs = new();
